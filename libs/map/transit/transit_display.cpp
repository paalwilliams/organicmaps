...
void TransitRouteInfo::AddStep(TransitStepInfo const & step)
{
  if (!m_steps.empty() && m_steps.back().IsEqualType(step))
  {
    ...
    // Extend the leg: keep the original boarding stop, demote the previous alighting stop
    // to an intermediate, and advance the alighting stop to the new edge's end stop.
                                                   ...
                                                   SubrouteParams & sp, SubrouteSegmentParams & ssp)
{
  ...
  auto const GetStopName = [&ssp](auto const & stop) -> std::string
  {
  ...
  m_routeInfo.AddStep(TransitStepInfo(sp.m_transitType, ssp.m_distance, ssp.m_time, line.GetNumber(),
                                      ColorToARGB(currentColor), 0 /* intermediateIndex */, GetStopName(stop1),
                                      GetStopName(stop2)));
